package com.mygdx.game.Screens;

import com.badlogic.gdx.Game;
import com.badlogic.gdx.Gdx;
import com.badlogic.gdx.Screen;
import com.badlogic.gdx.ScreenAdapter;
import com.badlogic.gdx.audio.Music;
import com.badlogic.gdx.graphics.Color;
import com.badlogic.gdx.graphics.Texture;
import com.badlogic.gdx.graphics.g2d.BitmapFont;
import com.badlogic.gdx.graphics.g2d.SpriteBatch;
import com.badlogic.gdx.graphics.g2d.freetype.FreeTypeFontGenerator;
import com.badlogic.gdx.scenes.scene2d.InputEvent;
import com.badlogic.gdx.scenes.scene2d.Stage;
import com.badlogic.gdx.scenes.scene2d.ui.Image;
import com.badlogic.gdx.scenes.scene2d.ui.Label;
import com.badlogic.gdx.scenes.scene2d.ui.Skin;
import com.badlogic.gdx.scenes.scene2d.ui.Table;
import com.badlogic.gdx.scenes.scene2d.ui.TextButton;
import com.badlogic.gdx.scenes.scene2d.utils.ActorGestureListener;
import com.badlogic.gdx.utils.I18NBundle;
import com.badlogic.gdx.utils.viewport.ScreenViewport;
import com.mygdx.game.Herramientas.ManagerAudio;
import com.mygdx.game.MyGdxGame;
import com.mygdx.game.Preferences;


/**
 * Clase que representa la pantalla del menu principal del juego.
 */
public class MainMenu extends ScreenAdapter {

    /**
     * Ancho de la pantalla.
     */
    float pantallaWidth = Gdx.graphics.getWidth();

    /**
     * Alto de la pantalla.
     */
    float pantallaHeight = Gdx.graphics.getHeight();

    /**
     * Objeto principal del juego.
     */
    private Game game;

    /**
     * Escenario donde se colocarán los elementos visuales.
     */
    private Stage stage;

    /**
     * Lote de sprites para dibujar elementos en la pantalla.
     */
    private SpriteBatch batch;

    /**
     * Conjunto de estilos para la interfaz gráfica.
     */
    private Skin skin;

    /**
     * Instancia del administrador de audio para gestionar la reproducción de sonidos y música.
     */
    private static ManagerAudio managerAudio;

    /**
     * Constructor de la clase MainMenu.
     *
     * @param game Objeto principal del juego.
     */
    public MainMenu(Game game){
        this.game=game;
        managerAudio=new ManagerAudio();
    }

    /**
     * Método para obtener la instancia del administrador de audio.
     *
     * @return Instancia del administrador de audio.
     */
    public static ManagerAudio getMangerAudio(){return managerAudio;}

    /**
     * Método llamado cuando la pantalla del menu principal se muestra por primera vez.
     */
    @Override
    public void show() {
        if (Preferences.getMusica()){
            managerAudio.startMusicMenu();
        }

        stage = new Stage(new ScreenViewport());
        Gdx.input.setInputProcessor(stage);
        skin=new Skin();

        batch = new SpriteBatch();

        float aspectRatio = pantallaWidth / pantallaHeight;

        int aspectRatioInt1 = (int) (aspectRatio * 35);
        int aspectRatioInt2 = (int) (aspectRatio * 60);

        Texture backgroundTexture = new Texture(Gdx.files.internal("recursos/fondoMenu.PNG"));
        Image backgroundImage = new Image(backgroundTexture);
        backgroundImage.setFillParent(true);
        stage.addActor(backgroundImage);

        FreeTypeFontGenerator generator = new FreeTypeFontGenerator(Gdx.files.internal("recursos/8bitlim.ttf"));
        FreeTypeFontGenerator.FreeTypeFontParameter parameter = new FreeTypeFontGenerator.FreeTypeFontParameter();
        parameter.size = aspectRatioInt1;
        BitmapFont bitF = generator.generateFont(parameter);

        FreeTypeFontGenerator.FreeTypeFontParameter parameterTitulo = new FreeTypeFontGenerator.FreeTypeFontParameter();
        parameterTitulo.size = aspectRatioInt2;
        BitmapFont bitFTitulo=generator.generateFont(parameterTitulo);
        generator.dispose();

        Label.LabelStyle estiloLabel= new Label.LabelStyle(bitFTitulo,Color.RED);

        TextButton.TextButtonStyle textButtonStyle = new TextButton.TextButtonStyle();
        textButtonStyle.font = bitF;
        textButtonStyle.fontColor = Color.WHITE;

        skin.add("default", textButtonStyle);

        Table tableTitulo=new Table();
        tableTitulo.top();
        tableTitulo.setFillParent(true);


        Table tabla = new Table();
        tabla.setFillParent(true);

        Label titulo=new Label(MyGdxGame.bundle.get("titulo"),estiloLabel);
        tableTitulo.add(titulo).expandX();

        TextButton botonJugar=new TextButton(MyGdxGame.bundle.get("jugar"),skin);
        botonJugar.addListener(new ActorGestureListener(){
            @Override
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {

                if (Preferences.getMusica()){
                    managerAudio.stopMusicMenu();
                }
                if (game instanceof MyGdxGame) {
                    MyGdxGame myGame = (MyGdxGame) game;
                    myGame.setScreen(new SeleccionMapaScreen(myGame));
                }
                dispose();
            }
        });

        TextButton botonOpciones=new TextButton(MyGdxGame.bundle.get("opciones"),skin);
        botonOpciones.addListener(new ActorGestureListener(){
            @Override
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                if (Preferences.getMusica()){
                    managerAudio.stopMusicMenu();
                }
                if (game instanceof MyGdxGame) {
                    MyGdxGame myGame = (MyGdxGame) game;
                    myGame.setScreen(new OptionsMenu(myGame));
                }
                dispose();
            }
        });

        TextButton botonRecords=new TextButton(MyGdxGame.bundle.get("records"),skin);
        botonRecords.addListener(new ActorGestureListener(){
            @Override
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                if (Preferences.getMusica()){
                    managerAudio.stopMusicMenu();
                }
                if (game instanceof MyGdxGame) {
                    MyGdxGame myGame = (MyGdxGame) game;
                    myGame.setScreen(new RecordsScreen(myGame));
                }
                dispose();
            }
        });

        TextButton botonCreditos=new TextButton(MyGdxGame.bundle.get("creditos"),skin);
        botonCreditos.addListener(new ActorGestureListener(){
            @Override
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                if (Preferences.getMusica()){
                    managerAudio.stopMusicMenu();
                }
                if (game instanceof MyGdxGame) {
                    MyGdxGame myGame = (MyGdxGame) game;
                    myGame.setScreen(new CreditosScreen(myGame));
                }
                dispose();
            }
        });

        TextButton botonTutorial=new TextButton("Tutorial",skin);
        botonTutorial.addListener(new ActorGestureListener(){
            @Override
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {

                if (Preferences.getMusica()){
                    managerAudio.stopMusicMenu();
                }
                if (game instanceof MyGdxGame) {
                    MyGdxGame myGame = (MyGdxGame) game;
                    myGame.setScreen(new TutorialScreen(myGame));
                }
                dispose();
            }
        });

        tabla.add(botonJugar).padBottom(20).row();
        tabla.add(botonOpciones).padBottom(20).row();
        tabla.add(botonRecords).padBottom(20).row();
        tabla.add(botonCreditos).padBottom(20).row();
        tabla.add(botonTutorial);

        stage.addActor(tableTitulo);
        stage.addActor(tabla);

    }

    /**
     * Método llamado en cada fotograma para renderizar la pantalla del menu principal.
     *
     * @param delta El tiempo transcurrido desde el último fotograma en segundos.
     */
    @Override
    public void render(float delta) {
        super.render(delta);
        stage.draw();
    }

    /**
     * Libera los recursos utilizados por la pantalla del menu principal cuando ya no es necesaria.
     */
    @Override
    public void dispose() {
        stage.dispose();
        batch.dispose();
        skin.dispose();
    }
}
